    ...
    <section className="section">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px', marginBottom: '32px' }}>
        {venues.map(v => {
          ...
          const allocation = get('layer2', `${v.key}-allocation`);
          const venue = get('layer2', `${v.key}-venue`);
          const pair = get('layer2', `${v.key}-pair`);
          const desc = get('layer2', `${v.key}-desc`);
          return (
            <div key={v.key} className="glass-card" style={{ padding: '24px', borderTop: `3px solid ${v.color}` }}>
              <div className="label" style={{ color: v.color, marginBottom: '12px' }}>{title}</div>
              <div className="anchor-stat" style={{ marginBottom: '6px' }}>{allocation}</div>
              {pair && <div className="label-sm" style={{ marginBottom: '4px' }}>{pair} · {venue}</div>}
              {!pair && <div className="label-sm" style={{ marginBottom: '4px' }}>{venue}</div>}
              <div className="divider" style={{ margin: '12px 0' }} />
              <p style={{ fontSize: '0.85rem', lineHeight: '1.55', color: 'var(--evro-shark-400)' }}>{desc}</p>
            </div>
          );
        })}
      </div>
